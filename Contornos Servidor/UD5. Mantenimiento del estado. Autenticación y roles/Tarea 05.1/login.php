...
//debug de session_start
$start = ini_get("session.auto_start");


require_once 'conexion.php';

//Función para mostrar mensajes de error y redirigir a la página de login
...
<?php
//Si el formulario de login se ha enviado
if (isset($_POST['login'])) {
    $nombre = trim($_POST['usuario']);
    $pass = trim($_POST['pass']);

    // Comprobamos que el nombre y la contraseña no tengan únicamente espacios en blanco
    ...
    //Consultamos a la base de datos para verificar las credenciales
    ...
    //Se comprueba que no hemos encontrado a ningún usuario que coincida con los datos obtenidos
    if ($stmt->rowCount()==0) {
        unset($_POST['login']);
        cerrarTodo($conProyecto, $stmt);
